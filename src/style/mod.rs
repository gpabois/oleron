...
use crate::{dom::TDocumentObjectModelExplorer, ecs::{component::Components, systems::tree::walk}};
...
/// ```
pub fn blockify<Dom>(dom: &Dom, node: &Dom::NodeId, style: &Style<Dom::NodeId>) 
where Dom: TDocumentObjectModelExplorer + Sync, Dom::NodeId: Hash + Copy + Eq
{
    let display = &mut style.computed.borrow_mut(node).unwrap().display;
