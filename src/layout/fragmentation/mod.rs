...
use crate::{r#box::Box, ecs::systems::{boxes::BoxSystem, tree::TreeSystem}};

use super::{fragment_tree::{Fragment, FragmentKind, FragmentTree}, Block, Inline, Lay, Layout};
...
pub fn fragment(fragment_tree: &mut FragmentTree, fragment: &Fragment) {
    ...
    let children = fragment_tree.iter_children(fragment).map(|child| (child, fragment_tree.is_break(fragment))).collect::<Vec<_>>();
    let mut previous= *fragment;
    let clusters = children.split(|(_, is_break)| *is_break).collect::<Vec<_>>();
...
impl Break<Inline> for OverflowBreak 
{
    fn r#break(&self, fragment_tree: &mut FragmentTree, fragment: &Fragment) -> bool {
        ...
        let child_boxes = fragment_tree
            ...
            .map(|child| (child, fragment_tree.is_break(&child), fragment_tree.borrow_box(&child).map(|c| c.clone())))
            .collect::<Vec<_>>();
        ...
        for (child, is_break, maybe_box) in child_boxes {
            ...
            if acc.content.width > self.max_length {
                ...
                } else { // we need to break before or after the monolith.
                    // We have fragments before we break it before
                    let maybe_at = if let Some(previous) = fragment_tree.before(&child) {
                        Some(previous)
                    } else if let Some(next) = fragment_tree.after(&child) { // We break afterwards
                        Some(next)
                    }  else {
                        None
                    };
